...
defaults = {
    ...
    "entry_capital": 0.0,
    "entry_time": None,
    "entry_balance": 1000.0,       # ✅ 자동손절 기준용 진입 시 잔고
    "leverage": 5,
    "position_ratio": 0.05,
    "trade_count": 0,
    "win": 0,
    ...
    "turn_ended": False,
    "price_range_result": None,    # ✅ 가격 폭 계산기 결과
}
...
def reset_position():
    ...
    st.session_state.entry_balance = st.session_state.balance  # 초기화 시 현재 잔고로 리셋
...
def open_position(pos, price, capital, leverage, position_ratio):
    ...
    st.session_state.entry_balance = st.session_state.balance  # ✅ 진입 시 잔고 저장 (자동손절 기준)
...
def check_liquidation(row):
    ...
    entry_balance = st.session_state.get("entry_balance", st.session_state.balance)
    ...
    # ── 2. 자동손절: 전체 시드 1% 초과손실 시점 가격 계산
    # 손절 조건: pnl = -entry_balance * 0.01
    # pnl = price_change * amt * lev  →  price_change = entry_balance * 0.01 / (amt * lev)
    if amt > 0:
        max_loss = entry_balance * 0.01
...
if st.session_state.turn_count >= MAX_TURNS:
    ...
    if st.button("🔁 새 매매 시작"):
        ...
        st.session_state.support_levels = []        # ✅ 지지선 리셋
        st.session_state.resistance_levels = []     # ✅ 저항선 리셋
        st.session_state.price_range_result = None  # ✅ 계산기 결과 리셋
        reset_position()
        st.session_state.turn_ended = False
        st.rerun()
...
if st.session_state.position is not None:
    ...
    amt = st.session_state.entry_capital
    lev = st.session_state.leverage
    entry_balance = st.session_state.entry_balance
    ...
    # ✅ 자동손절가 계산 (시드 1% 기준)
    if amt > 0:
        price_change_stop = (entry_balance * 0.01) / (amt * lev)
        if st.session_state.position == "LONG":
...
- 🛑 자동손절가 <span style="font-size:0.85em;color:gray;">(시드 1%)</span>: <span style="color:#e05c5c;font-weight:bold;">{f'{auto_stop_price:,.2f}' if auto_stop_price else 'N/A'}</span>
- 진입 금액: **${amt:,.2f}**
- 레버리지: **{lev}x**
- 손익률 (레버리지):
...
st.sidebar.info("레버리지: **20x** (고정)  |  자동손절: **시드 1%**")
...
# 📐 가격 폭 계산기
# =====================
st.sidebar.subheader("📐 가격 폭 계산기")
price_a = st.sidebar.number_input("시작 가격", value=0.0, step=1.0, key="calc_price_a")
price_b = st.sidebar.number_input("끝 가격", value=0.0, step=1.0, key="calc_price_b")

col_calc1, col_calc2 = st.sidebar.columns(2)
if col_calc1.button("📊 계산"):
    if price_a > 0 and price_b > 0:
        pct = abs(price_b - price_a) / price_a * 100
        direction_str = "📈 상승" if price_b > price_a else "📉 하락"
        st.session_state.price_range_result = (
            f"{direction_str}\n"
            f"{price_a:,.2f} → {price_b:,.2f}\n"
            f"폭: **{pct:.2f}%**"
        )
    else:
        st.session_state.price_range_result = "❗ 유효한 가격을 입력하세요."

if col_calc2.button("🗑 지우기"):
    st.session_state.price_range_result = None

if st.session_state.get("price_range_result"):
    st.sidebar.success(st.session_state.price_range_result)
