...
def check_liquidation(row):
    ...
    # ── 2. 자동손절: 현재 잔고 1% 초과손실 시점 가격 계산
    # pnl = price_change * amt * lev = -현재잔고 * 0.01
    if amt > 0:
        max_loss = st.session_state.balance * 0.01
...
if st.session_state.position is not None:
    ...
    # ✅ 자동손절가 계산 (현재 잔고 1% 기준)
    if amt > 0:
        price_change_stop = (st.session_state.balance * 0.01) / (amt * lev)
        if st.session_state.position == "LONG":
...
- 🛑 자동손절가 <span style="font-size:0.85em;color:gray;">(현재잔고 1%)</span>: <span style="color:#e05c5c;font-weight:bold;">{f'{auto_stop_price:,.2f}' if auto_stop_price else 'N/A'}</span>
- 진입 금액: **${amt:,.2f}**
- 레버리지: **{lev}x**
- 손익률 (레버리지):
